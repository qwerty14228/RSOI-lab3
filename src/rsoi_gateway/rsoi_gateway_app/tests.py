...
class LibraryClientTestCase(TestCase):
    ...
    def test_get_library_books(self):
        ...
        for item in data['items']:
            ...
            self.assertIn('library', item)
            self.assertIn('book_uid', item['book'])
            self.assertIn('name', item['book'])
            self.assertIn('author', item['book'])
            self.assertIn('genre', item['book'])
            self.assertIn('condition', item['book'])
            self.assertIn('library_uid', item['library'])
            self.assertIn('name', item['library'])
            self.assertIn('city', item['library'])
            self.assertIn('address', item['library'])
            self.assertIn('available_count', item)
